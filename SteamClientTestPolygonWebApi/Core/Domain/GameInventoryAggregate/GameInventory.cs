...
namespace SteamClientTestPolygonWebApi.Core.Domain.GameInventoryAggregate;
...
public class GameInventory
{
    ...
    private static void EnforceAppIdInvariant(GameInventoryAsset asset, int appId)
    {
        if (asset.AppId != appId)
            throw new ArgumentException("Asset must have the same AppId as the inventory.", nameof(asset.AppId));
    }
